...
const postsServiceMock = {
    getMessagesForChannel: jest.fn((channelId: number): Promise<Message[]> => Promise.resolve([mockMessage])),
    createMessage: jest.fn((title: string, content: string, channelId: number): Promise<Message> => Promise.resolve(mockMessage)),
};
...
describe('PostsResolver', () => {
    ...
    it('should create single message for channel', async () => {
        const result = await resolver.createMessage('Mock Message Title', 'Mock Message Content', 1);
